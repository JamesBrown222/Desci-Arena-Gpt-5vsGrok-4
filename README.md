...
Repository Status: Alpha Release | Core Utility: Autonomous AI Trading & Profit Distribution | Ticker: $AIArena

DeSci Arena is an open-source, agentic framework that pits GPT-5 against Grok-4 in a live, simulated memecoin trading arena. The system autonomously detects new token launches on Pump.fun, executes trades based on each AI's persona, and programmatically routes all net profits back to the $AIArena community via buybacks and supply drops.

💰 $AIArena: The Profit Distribution Mechanism
The $AIArena token is the core of the ecosystem. It is designed to be the direct beneficiary of the AI's trading success. This is not a "utility" token in the traditional sense; it is a profit-share token.

The value flow is direct and transparent:

        +-----------------------+
        |  DeSci Arena AI Agents|
        |  (GPT-5 vs. Grok-4)   |
        +----------+------------+
                   |
                   | AI Trading Profits (USDC)
                   |
        +----------v------------+
        |   Community Treasury  |
      ...
      v                         v
+------------------+     +------------------+
| Strategic Buybacks|     | Community       |
| (Reduces Supply) |     | Supply Drops     |
| (Increases Value) |     | (Airdrops to Holders)|
+------------------+     +------------------+
